...
class PureStrategy(Policy):
    """
    Pure strategy class.
    """

    def __init__(self, action_space: list[ActionTargetPair], pure_strategy: list[ActionTargetPair]) -> None:
        """
        Initialize the pure strategy with a list of action-target pairs.
        """
        super().__init__(action_space)
    ...
    def get_probs(self, extended_observations: list[PlayerTargetPair | None] | None = None) -> list[float]:
        m = len(self._extend_observations(extended_observations))
        action = self.pure_strategy[m]

        return [1.0 if i == action else 0.0 for i in self.action_space]


class MixedStrategy(Policy):
    """
    Mixed strategy class.
    """

    def __init__(self, action_space: list[ActionTargetPair], pure_strategies: list[list[ActionTargetPair]], probabilities: list[float]) -> None:
        """
        Initialize the mixed strategy with a list of probabilities.
        """
        super().__init__(action_space)
    ...
    def get_probs(self, extended_observations: list[PlayerTargetPair | None] | None = None) -> list[float]:
        m = len(self._extend_observations(extended_observations))
        probs = torch.zeros_like(torch.tensor(self.action_space), dtype=torch.float32)

        for i, pure_strategy in enumerate(self.pure_strategies):
            action = pure_strategy[m]
            probs[self.action_space.index(action)] += self.probabilities[i]
    ...
    def simplify(self, min_prob_threshold: float = 1e-4, max_pure_strategies: int = 50) -> None:
        ...
        strategy_map: dict[tuple[ActionTargetPair, ...], float] = {}
        for i, pure_strat in enumerate(self.pure_strategies):
            strat_tuple = tuple(pure_strat)
        ...
        for strat_tuple, prob in strategy_map.items():
            if prob >= min_prob_threshold:
                filtered_strategies.append(list(strat_tuple))
                filtered_probabilities.append(prob)

        if len(filtered_strategies) == 0:  # Avoid empty strategy if all below threshold
            ...
            if strategy_map:
                ...
                filtered_strategies = [list(best_strat_tuple)]
                filtered_probabilities = [1.0]  # Normalize to 1
        ...
        if len(filtered_strategies) > max_pure_strategies:
            # Sort by probability and keep the top ones
            indices = np.argsort(filtered_probabilities)[::-1][:max_pure_strategies]
            filtered_strategies = [filtered_strategies[i] for i in indices]
            filtered_probabilities = [filtered_probabilities[i] for i in indices]

        # --- Renormalize ---
        prob_sum = sum(filtered_probabilities)
        if prob_sum > 0:  # Avoid division by zero if list becomes empty
            self.probabilities = [p / prob_sum for p in filtered_probabilities]
            self.pure_strategies = filtered_strategies
