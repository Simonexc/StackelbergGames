...
from .simple_nn import CombinedPolicy
...
class PureStrategy(StrategyBase):
    ...
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        embedding = torch.zeros(self.pure_strategy.shape[0], dtype=torch.float32, device=tensordict.device)
...
def _worker_evaluate_strategy(args_tuple):
    ...
    # 1. Reconstruct Environment
    # Assuming FlipItEnv can be reconstructed from a dictionary of its parameters
    current_env = FlipItEnv(env_map, env_num_steps)

    # 2. Reconstruct Opponent Population
    # opponent_population = []
    # for opp_config in opponent_population_configs:
    #     action_size_opp = opp_config.pop("action_size")  # Extract action_size
    #     opp_strat_type_str = opp_config.pop("strategy_type")  # Extract strategy type
    #
    #     if opp_strat_type_str == "PureStrategy":
    #         opp_strategy = PureStrategy(action_size_opp, **opp_config)
    #     elif opp_strat_type_str == "MixedStrategy":
    #         # MixedStrategy reconstruction is more complex if pure_strategies are nested
    #         # This simplified config passing assumes pure_strategies are part of opp_config
    #         # For robust reconstruction, pass data for PureStrategy components separately.
    #         # The current PureStrategy/MixedStrategy save/load is instance based.
    #         # For multiprocessing, better to serialize to basic data.
    #         # This part is a simplification and might need refinement for MixedStrategy reconstruction.
    #         # Let's assume opponent_population_configs contains already reconstructed strategies for now.
    #         # This means the main process did thedeepcopying or they are picklable.
    #         # The provided args_tuple passes opponent_population_list directly for now for simplicity.
    #         # A more robust version would serialize/deserialize.
    #         raise NotImplementedError("Reconstruction of MixedStrategy from config in worker needs more detail.")
    #     else:
    #         raise ValueError(f"Unknown opponent strategy type: {opp_strat_type_str}")
    #     opponent_population.append(opp_strategy)

    # Ensure _env_instance is a fresh copy if it has state, or use env_config_dict to make one
    # For now, assume _env_instance is okay to use as is (e.g., stateless for single_run or copied).
...
class CoevoSGAgentBase(BaseAgent, ABC):
    def __init__(
        ...
        super().__init__(action_size, player_type, embedding_size, device, run_name, agent_id)
